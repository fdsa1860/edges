...
function forest = forestTrain( data, hs, hs2, varargin )
...
assert(length(hs2)==N); % added by xikang
...
%   tree = treeTrain(data1,hs1,dWts1,prmTree);
  if(N==N1), data1=data; hs1=hs; dWts1=dWts; else
    d=wswor(dWts,N1,4); data1=data(d,:); hs1=hs(d);hs2=hs2(d);
    dWts1=dWts(d); dWts1=dWts1/sum(dWts1);
  end
  tree = treeTrain(data1,hs1,hs2,dWts1,prmTree);
...
function tree = treeTrain( data, hs, hs2, dWts, prmTree )
...
%   if(discr), [hs1,hsn{k}]=feval(discretize,hs1,H); hs1=uint32(hs1); end
  if(discr), [hs1,~,hsn{k}]=feval(discretize,hs1,hs2(dids1),H); hs1=uint32(hs1); end
